 ...
 */
public class Single<T1> extends Tuple implements IImmutableSingle<T1> {

    /**
     * The element of the single.
     */
    final public T1 _1;

    /**
     * Constructs a immutable single.
     *
    ...
    @Override
    public Single<T1> _1(T1 _1) {
        return new Single<>(_1);
    }

    @Override
    public Single<T1> first(T1 first) {
    ...
    /**
     * Returns a immutable single.
     *
     * @param _1 the first element
     * @param <T1> the class of the first element
     * @return a {@code ImmutableSingle}
     */
    ...
    public static <T> Single<T> from(T[] array) {
        return from(Arrays.asList(Objects.requireNonNull(array)));
    }
